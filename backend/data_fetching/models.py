...
class UserFavoriteLocation(models.Model):
    user_id = models.ForeignKey(User, on_delete=models.CASCADE, related_name="favorite_locations")
    location_id = models.ForeignKey(Location, on_delete=models.CASCADE, related_name="favorited_by")

    class Meta:
        unique_together = ('user_id', 'location_id')

    def __str__(self):
        return f"{self.user_id.username} → {self.location_id.city}"
...
class MLRecommendation(models.Model):
    id = models.AutoField(primary_key=True)
    user_id = models.ForeignKey(User, on_delete=models.CASCADE, related_name="ml_recommendations")
    location_id = models.ForeignKey(Location, on_delete=models.CASCADE, related_name="ml_recommendations")
    recommendation_id = models.IntegerField()
    trigger_reason = models.CharField(max_length=255)
    generated_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Recommendation {self.recommendation_id} for {self.user_id.username}"
